function [u] = MiniBatchOjaSynthetic_Final_B1000_SaveAll
...
A = diag([(1/(dataDimension - 5)):(1/(dataDimension - 5)):1, zeros(1, 5)]); % Gap = 0.2
ER = A*ER;
a = mean(ER, 2);
...
for simulationRun=1:TotalSimulationRuns
    ...
    for i=1:length(gamma_vary)
        ...
        if ~sum(isnan(ErrorMini))
            ErrorMiniBatchOja(:, simulationRun) = ErrorMiniBatchOja(:, simulationRun)+ErrorMini;
...
%% Saving results
tic
save -v7.3 ErrorKrasulinaMiniBatch_Final_B_1000_d10_All ErrorMiniBatchOja TotalConvergingRuns
toc
end