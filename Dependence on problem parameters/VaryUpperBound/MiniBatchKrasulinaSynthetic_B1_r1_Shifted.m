function [u] = MiniBatchOjaSynthetic_Final_B1_r1_Shifted
...
gamma = 20;
shift = 0:50:200;
ErrorMiniBatchOja = zeros(TotalIterations + 1, length(shift));
TotalConvergingRuns = zeros(length(shift), 1);
r = 1; %Upper bound
%% Generate data
ER = -r + 2*r*rand(dataDimension, TotalSamples);% Uniform dist
...
for simulationRun=1:TotalSimulationRuns
    ...
    for i=1:length(shift)
        ...
        for k=1:TotalIterations/batch
            ...
            x_t = x_t + ((gamma)/(k+shift(i)))*(CovarianceBatch*x_t - (x_t'*CovarianceBatch*x_t)*x_t/(x_t'*x_t));
    ...
    %% Saving results
    tic
    save -v7.3 ErrorKrasulinaMiniBatch_Final_B_1_r1_Shifted ErrorMiniBatchOja TotalConvergingRuns maxNorm
    toc
end
end