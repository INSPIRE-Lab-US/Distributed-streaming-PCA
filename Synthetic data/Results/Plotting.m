...
h1 = loglog(ErrorMiniBatchOja(:, 4)/TotalConvergingRuns(4), '-^', 'LineWidth', 2, 'MarkerIndices', ceil(10.^(0:0.5:4)), 'MarkerSize', 8)
...
hold on, loglog(ErrorMiniBatchOja(1:end, 5)/TotalConvergingRuns(5), '-o', 'LineWidth', 2, 'MarkerIndices', ceil(10.^(0:0.5:4)), 'MarkerSize', 8)
...
hold on, loglog(ErrorMiniBatchOja(1:end, 3)/TotalConvergingRuns(3), '--*', 'LineWidth', 2, 'MarkerIndices', ceil(10.^(0:0.5:4)), 'MarkerSize', 8)
%loglog(1:length(ErrorMiniBatchOja(:, 1)), 1./(1:length(ErrorMiniBatchOja(:, 1))), '--', 'LineWidth', 1)
xlabel('$\#$ of samples (T) received', 'interpreter', 'Latex')
ylabel('$1 - \frac{\mathbf{q}{^{*^T}} \mathbf{v}_t}{\|\mathbf{v}_t\|_2^2}$', 'interpreter', 'Latex')
h = legend('$B=1$', '$B=10$', '$B=100$', '$B=500$', '$B=1000$', '$B=2000$', 'location','SouthWest');
set(h, 'interpreter', 'Latex')
set(gca, 'FontName', 'Arial', 'FontSize', 20)
print -depsc ?SyntheticPCA_NoLatency.eps