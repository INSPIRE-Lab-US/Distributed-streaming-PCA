...
h1 = loglog(ErrorMiniBatchOja(:, 5)/TotalConvergingRuns(5), '-^', 'LineWidth', 2, 'MarkerIndices', ceil(10.^(0:0.5:4)), 'MarkerSize', 8)
...
load('ErrorKrasulinaMiniBatch_Final_B_300.mat')
hold on, loglog(ErrorMiniBatchOja(1:end, 5)/TotalConvergingRuns(5), '-*', 'LineWidth', 2, 'MarkerIndices', ceil(10.^(0:0.5:4)), 'MarkerSize', 8)
load('ErrorKrasulinaMiniBatch_Final_B_1000.mat')
hold on, loglog(ErrorMiniBatchOja(1:end, 5)/TotalConvergingRuns(5), '-*', 'LineWidth', 2, 'MarkerIndices', ceil(10.^(0:0.5:4)), 'MarkerSize', 8)
%loglog(1:length(ErrorMiniBatchOja(:, 1)), 1./(1:length(ErrorMiniBatchOja(:, 1))), '--', 'LineWidth', 1)
xlabel('$\#$ of samples ($T$) received', 'interpreter', 'Latex')
ylabel('$1 - \frac{(\mathbf{q}^{*^T} \mathbf{v}_t)^2}{\|\mathbf{v}_t\|_2^2}$', 'interpreter', 'Latex')
h = legend('$B=1$', '$B=10$', '$B=100$', '$B=300$', '$B=1000$', 'location','SouthWest');
set(h, 'interpreter', 'Latex')
set(gca, 'FontName', 'Arial', 'FontSize', 20)
print -depsc MNIST_PCA_NoLatency.eps